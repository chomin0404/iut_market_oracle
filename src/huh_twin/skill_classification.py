...
SKILL_CLASSIFICATION: Final[tuple[ClassifiedSkill, ...]] = (
    ClassifiedSkill(
        "フェルミ推定", SkillBasis.PROBABILISTIC_INFERENCE, "粗い事前分布と数量オーダーの初期化"
    ),
    ClassifiedSkill("ベイズ統計", SkillBasis.PROBABILISTIC_INFERENCE, "事前・事後更新の中核"),
    ClassifiedSkill(
        "ベイジアンネット（ベイジアン最適化）",
        SkillBasis.PROBABILISTIC_INFERENCE,
        "依存構造と逐次最適化",
    ),
    ClassifiedSkill(
        "確率分布（パレート分布）", SkillBasis.PROBABILISTIC_INFERENCE, "裾の厚い事象のモデリング"
    ),
    ClassifiedSkill(
        "ガンマ・ポアソン分布", SkillBasis.PROBABILISTIC_INFERENCE, "カウント過程と事後更新"
    ),
    ClassifiedSkill("ラプラス分布", SkillBasis.PROBABILISTIC_INFERENCE, "ロバスト誤差と尖度"),
    ClassifiedSkill(
        "NBDモデル", SkillBasis.PROBABILISTIC_INFERENCE, "反復回数と顧客行動の確率モデル"
    ),
    ClassifiedSkill("コーシー分布", SkillBasis.PROBABILISTIC_INFERENCE, "極端値とロバスト推定"),
    ClassifiedSkill(
        "ディリクレ分布", SkillBasis.PROBABILISTIC_INFERENCE, "混合比率とカテゴリ事前分布"
    ),
    ClassifiedSkill(
        "具体的な出口戦略（EXIT）の数理モデル",
        SkillBasis.VALUATION_AND_CAPITAL_ALLOCATION,
        "保有・売却・段階的EXITの定量比較",
    ),
    ...
    ClassifiedSkill(
        "競争優位性", SkillBasis.VALUATION_AND_CAPITAL_ALLOCATION, "超過収益持続性の源泉"
    ),
    ClassifiedSkill(
        "Moat（経済的な堀）", SkillBasis.VALUATION_AND_CAPITAL_ALLOCATION, "持続的優位性の評価軸"
    ),
    ClassifiedSkill(
        "Sensitivity Analysis（感応度分析）",
        SkillBasis.STRATEGY_AND_IMPLEMENTATION,
        "価値ドライバーの識別",
    ),
    ClassifiedSkill(
        "MCMCマルコフ連鎖モンテカルロ・シミュレーション",
    ...
    ClassifiedSkill(
        "エントロピー理論", SkillBasis.OPTIMAL_EXPERIMENT_DESIGN, "情報量・不確実性の測定"
    ),
    ClassifiedSkill(
        "ビジネスモデル", SkillBasis.STRATEGY_AND_IMPLEMENTATION, "収益機構と実装対象の定義"
    ),
    ClassifiedSkill(
        "成長率10%", SkillBasis.VALUATION_AND_CAPITAL_ALLOCATION, "ベースケースの成長仮定"
    ),
    ClassifiedSkill(
        "割引率10%", SkillBasis.VALUATION_AND_CAPITAL_ALLOCATION, "ベースケースの資本コスト仮定"
    ),
    ClassifiedSkill(
        "量子化学計算", SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION, "物性・反応のシミュレーション"
    ),
    ...
    ClassifiedSkill(
        "デジタルツイン", SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION, "状態空間の複製と更新"
    ),
    ClassifiedSkill(
        "仮想空間シミュレーション",
        SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION,
        "介入前評価と反実仮想",
    ),
    ClassifiedSkill(
        "Claudecodeの活用", SkillBasis.STRATEGY_AND_IMPLEMENTATION, "研究OSと実装自動化"
    ),
    ClassifiedSkill(
        "シグモイド曲線", SkillBasis.STRATEGY_AND_IMPLEMENTATION, "普及・成長段階の認識"
    ),
    ClassifiedSkill(
        "アナロジー", SkillBasis.STRATEGY_AND_IMPLEMENTATION, "異分野写像による仮説生成"
    ),
    ClassifiedSkill(
        "ゲーム理論（ナッシュ均衡）", SkillBasis.STRATEGY_AND_IMPLEMENTATION, "競争環境の均衡分析"
    ),
    ClassifiedSkill(
        "SOTP（Sum-of-the-Parts：事業別評価合算）分析",
        SkillBasis.VALUATION_AND_CAPITAL_ALLOCATION,
    ...
    ClassifiedSkill(
        "因果推論（Causal Inference）", SkillBasis.PROBABILISTIC_INFERENCE, "介入効果の識別"
    ),
    ClassifiedSkill(
        "虚数時間", SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION, "解析接続と安定化の道具"
    ),
    ClassifiedSkill(
        "フィールズ賞（歴代フィールズ賞受賞者の論文）",
        SkillBasis.STRATEGY_AND_IMPLEMENTATION,
    ...
    ClassifiedSkill(
        "Lax-Phillips基盤", SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION, "散乱理論と共鳴解析"
    ),
    ClassifiedSkill(
        "ラドン変換（Radon Transform）",
        SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION,
    ...
    ClassifiedSkill(
        "フィッシャー情報行列式を最大化（D-Optimality）",
        SkillBasis.OPTIMAL_EXPERIMENT_DESIGN,
        "識別力最大化の実験設計",
    ),
    ClassifiedSkill(
        "感応度行列（Jacobian）", SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION, "局所感度と線形化"
    ),
    ClassifiedSkill(
        "特異摂動解析", SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION, "多重スケール系の分離"
    ),
    ClassifiedSkill(
        "メトロポリス・ヘイスティングス法（MH法）",
        SkillBasis.PROBABILISTIC_INFERENCE,
    ...
    ClassifiedSkill(
        "ブライト・ウィグナー分布", SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION, "共鳴幅と散乱ピーク"
    ),
    ClassifiedSkill(
        "ブライト・ウィグナー分布",
        SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION,
    ...
    ClassifiedSkill(
        "Hilbert空間", SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION, "作用素・射影・観測表現の基盤"
    ),
    ClassifiedSkill(
        "量子共鳴解析", SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION, "準束縛状態とスペクトル構造"
    ),
    ClassifiedSkill(
        "成長断片化方程式の確率論的アプローチ",
        SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION,
    ...
    ClassifiedSkill(
        "レジーム転換モデル", SkillBasis.PROBABILISTIC_INFERENCE, "状態遷移と構造変化の推定"
    ),
    ClassifiedSkill(
        "もつれ状態のコヒーレンス崩壊",
        SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION,
    ...
    ClassifiedSkill(
        "2DMAT", SkillBasis.OPTIMAL_EXPERIMENT_DESIGN, "材料探索・設計空間探索の実装系"
    ),
    ClassifiedSkill(
        "極値関数をどう設計するか", SkillBasis.OPTIMAL_EXPERIMENT_DESIGN, "目的関数と探索戦略の設計"
    ),
    ClassifiedSkill(
        "Lippmann-Schwinger",
        SkillBasis.INVERSE_PROBLEMS_AND_SIMULATION,
