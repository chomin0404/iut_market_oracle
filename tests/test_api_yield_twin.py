...
def test_recommend_status_ok() -> None:
    resp = client.post("/api/v1/yield-twin/recommend", json=_BASE_PAYLOAD)
    assert resp.status_code == 200


def test_recommend_returns_doe_recommendation_fields() -> None:
    body = client.post("/api/v1/yield-twin/recommend", json=_BASE_PAYLOAD).json()
...
def test_recommend_no_observations_doe_explore_mode() -> None:
    ...
    body = client.post("/api/v1/yield-twin/recommend", json=payload).json()
    assert body["acquisition_mode"] == "doe_explore"
    assert body["n_observations"] == 0


def test_recommend_reproducible_with_same_seed() -> None:
    r1 = client.post("/api/v1/yield-twin/recommend", json=_BASE_PAYLOAD).json()
    r2 = client.post("/api/v1/yield-twin/recommend", json=_BASE_PAYLOAD).json()
...
def test_recommend_different_seed_may_differ() -> None:
    ...
    r1 = client.post("/api/v1/yield-twin/recommend", json=_BASE_PAYLOAD).json()
    r2 = client.post("/api/v1/yield-twin/recommend", json=payload_b).json()
...
def test_report_status_ok() -> None:
    resp = client.post("/api/v1/yield-twin/report", json=_BASE_PAYLOAD)
    assert resp.status_code == 200


def test_report_fields() -> None:
    body = client.post("/api/v1/yield-twin/report", json=_BASE_PAYLOAD).json()
    assert "n_observations" in body
...
def test_report_single_observation_no_loocv() -> None:
    ...
    body = client.post("/api/v1/yield-twin/report", json=payload).json()
...
def test_missing_factor_specs_returns_422() -> None:
    resp = client.post("/api/v1/yield-twin/recommend", json={"observations": []})
...
def test_duplicate_factor_names_returns_422() -> None:
    ...
    resp = client.post("/api/v1/yield-twin/recommend", json=payload)
    assert resp.status_code == 422
