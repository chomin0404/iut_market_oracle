...
class TestLogConcavityStatus:
    def test_status_200(self):
        r = client.post("/api/v1/matroid/log-concavity", json=_PAYLOAD)
    ...
    def test_n_assets_above_max_returns_422(self):
        r = client.post("/api/v1/matroid/log-concavity", json={**_PAYLOAD, "n_assets": 201})
        assert r.status_code == 422

    def test_n_assets_zero_returns_422(self):
        r = client.post("/api/v1/matroid/log-concavity", json={**_PAYLOAD, "n_assets": 0})
        assert r.status_code == 422

    def test_rank_weight_zero_returns_422(self):
        r = client.post("/api/v1/matroid/log-concavity", json={**_PAYLOAD, "rank_weight": 0.0})
        assert r.status_code == 422

    def test_corank_weight_negative_returns_422(self):
        r = client.post("/api/v1/matroid/log-concavity", json={**_PAYLOAD, "corank_weight": -1.0})
        assert r.status_code == 422

    def test_default_weights_applied(self):
        r = client.post("/api/v1/matroid/log-concavity", json={"n_assets": 5})
        assert r.status_code == 200
        body = r.json()
...
class TestLogConcavityParametric:
    ...
    def test_n_assets_1_edge_case(self):
        r = client.post("/api/v1/matroid/log-concavity", json={"n_assets": 1})
        body = r.json()
    ...
    def test_idempotent(self):
        ...
        r1 = client.post("/api/v1/matroid/log-concavity", json=_PAYLOAD)
        r2 = client.post("/api/v1/matroid/log-concavity", json=_PAYLOAD)
