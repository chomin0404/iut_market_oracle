...
class TestRegimeSimulate:
    ...
    def test_reproducibility(self):
        r1 = client.post("/api/v1/twin/regime-simulate", json=_REGIME_PAYLOAD)
        r2 = client.post("/api/v1/twin/regime-simulate", json=_REGIME_PAYLOAD)
        assert r1.json()["prices"] == r2.json()["prices"]

    def test_different_seeds_differ(self):
        r1 = client.post("/api/v1/twin/regime-simulate", json={**_REGIME_PAYLOAD, "random_seed": 1})
        r2 = client.post("/api/v1/twin/regime-simulate", json={**_REGIME_PAYLOAD, "random_seed": 2})
        assert r1.json()["prices"] != r2.json()["prices"]

    def test_n_steps_above_max_returns_422(self):
        r = client.post("/api/v1/twin/regime-simulate", json={**_REGIME_PAYLOAD, "n_steps": 9999})
        assert r.status_code == 422

    def test_missing_random_seed_returns_422(self):
        payload = {k: v for k, v in _REGIME_PAYLOAD.items() if k != "random_seed"}
        r = client.post("/api/v1/twin/regime-simulate", json=payload)
        assert r.status_code == 422

    def test_p_stay_normal_zero_returns_422(self):
        payload = {**_REGIME_PAYLOAD, "p_stay_normal": 0.0}
        r = client.post("/api/v1/twin/regime-simulate", json=payload)
        assert r.status_code == 422

    def test_initial_price_zero_returns_422(self):
        payload = {**_REGIME_PAYLOAD, "initial_price": 0.0}
        r = client.post("/api/v1/twin/regime-simulate", json=payload)
        assert r.status_code == 422
...
class TestRegimeSimulateSummary:
    ...
    def test_summary_consistent_with_full(self):
        ...
        full = client.post("/api/v1/twin/regime-simulate", json=_REGIME_PAYLOAD).json()
        summary = client.post("/api/v1/twin/regime-simulate/summary", json=_REGIME_PAYLOAD).json()
    ...
    def test_reproducibility(self):
        r1 = client.post("/api/v1/twin/regime-simulate/summary", json=_REGIME_PAYLOAD)
        r2 = client.post("/api/v1/twin/regime-simulate/summary", json=_REGIME_PAYLOAD)
...
class TestMarketEvolve:
    ...
    def test_reproducibility(self):
        r1 = client.post("/api/v1/twin/market-evolve", json=_MARKET_PAYLOAD)
        r2 = client.post("/api/v1/twin/market-evolve", json=_MARKET_PAYLOAD)
        assert r1.json()["new_customers"] == r2.json()["new_customers"]

    def test_different_seeds_differ(self):
        r1 = client.post("/api/v1/twin/market-evolve", json={**_MARKET_PAYLOAD, "random_seed": 1})
        r2 = client.post("/api/v1/twin/market-evolve", json={**_MARKET_PAYLOAD, "random_seed": 2})
        assert r1.json()["new_customers"] != r2.json()["new_customers"]

    def test_n_steps_above_max_returns_422(self):
        r = client.post("/api/v1/twin/market-evolve", json={**_MARKET_PAYLOAD, "n_steps": 9999})
        assert r.status_code == 422

    def test_missing_random_seed_returns_422(self):
        payload = {k: v for k, v in _MARKET_PAYLOAD.items() if k != "random_seed"}
        r = client.post("/api/v1/twin/market-evolve", json=payload)
        assert r.status_code == 422

    def test_gamma_alpha_zero_returns_422(self):
        r = client.post("/api/v1/twin/market-evolve", json={**_MARKET_PAYLOAD, "gamma_alpha": 0.0})
        assert r.status_code == 422
